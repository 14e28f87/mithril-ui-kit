 ...
 *
 * `ModalRoot.view()` または `ModalImperativeWrapper.view()` 内で
 * モジュールスコープの `_modalCtx` にセットされる。
 * Mithril の同期レンダリングサイクルにより、同じ描画サイクル内で
 * 子コンポーネントの `view()` が実行されるときに読み取れる。
 */
interface ModalContext {
	/** モーダルを閉じる関数（アニメーション付き） */
	close: () => void;
	/** 外側（ポジショナー背景）クリックで閉じるか */
	closeOnInteractOutside: boolean;
	/** 表示アニメーション（enter）実行中かどうか */
	entering: boolean;
	/** 非表示アニメーション（exit）実行中かどうか */
	exiting: boolean;
	/** アニメーションプリセット（"scale" | "none"） */
	motionPreset: ModalMotionPreset;
	/** モーダルのサイズ */
	size: ModalSize;
	/** モーダルの縦方向配置 */
	placement: ModalPlacement;
	/** スクロール動作（内側スクロールか外側スクロールか） */
	scrollBehavior: ModalScrollBehavior;
}

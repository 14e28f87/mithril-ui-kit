...
export class AccordionClassic {
    ...
    currentKeys(attrs) {
        if (attrs.activeKeys)
            return new Set(attrs.activeKeys);
     ...
     * @private
     */
    focusHeaderByDelta(delta) {
        if (!this.container)
            return;
        const headers = Array.from(this.container.querySelectorAll("[data-accordion-button]"));
        if (!headers.length)
            return;
    ...
    view(vnode) {
        ...
        return (m("div", { class: rootClass, style: attrs.style, oncreate: (d) => (this.container = d.dom), onremove: () => (this.container = undefined) }, items.map((it, index) => {
            ...
            return (m("div", { class: styles.accordionItem, key: String(key) },
                m("div", { class: styles.accordionHeader, id: panelId },
                    m("button", { class: classNames(styles.accordionButton, { [styles.accordionButtonCollapsed]: !open, disabled: !!it.disabled }), type: "button", "data-accordion-button": "", "aria-expanded": open, "aria-controls": collapseId, onclick: () => this.toggle(vnode, key, it.disabled), onkeydown: (e) => {
                            if (e.key === "ArrowDown") {
                                e.preventDefault();
                ...
                m("div", { id: collapseId, class: classNames(styles.accordionCollapse, { [styles.accordionCollapseHidden]: !open }), "aria-labelledby": panelId },
                    m("div", { class: styles.accordionBody }, it.content))));
        })));
    }
}
AccordionClassic.counter = 0;

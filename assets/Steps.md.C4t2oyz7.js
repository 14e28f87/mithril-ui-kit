...
`,h=`/** @jsx m */\r
import m from "mithril";\r
import { Steps, Button } from "mithril-ui-kit";\r
\r
export function setup(el: HTMLElement): void {\r
  let step = 1;\r
      ...
      return (\r
        <div style={{ display: "grid", gap: "14px" }}>\r
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>\r
            {[0, 1, 2].map((index) => (\r
              <Button onclick={() => { step = index; completed = false; m.redraw(); }}>\r
                Step {index + 1}\r
              </Button>\r
            ))}\r
            <Button onclick={() => { step = 0; completed = false; m.redraw(); }}>Reset</Button>\r
          </div>\r
\r
          <Steps.Root\r
            count={3}\r

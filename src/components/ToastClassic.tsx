...
type ToastWrapperAttrs = {
	hide: () => void;
	options: ToastClassicOptions;
	closing: boolean;
	duration: number;
};
...
export class ToastClassic {
	 ...
	 * @private
	 */
	private static containers: Map<ToastClassicPosition, HTMLElement> = new Map();

	/**
	 * 指定位置のコンテナ要素を取得または作成
	 * @private
	 */
	private static getContainerForPosition(position: ToastClassicPosition): HTMLElement {
	...
	static show<T = void>(opts: ToastClassicOptions<T>): Promise<T> {
		return new Promise<T>((resolve) => {
			 ...
			 * - toast-body: コンテンツ
			 */
			class ToastWrapper implements m.ClassComponent<ToastWrapperAttrs> {
				view(vnode: m.Vnode<ToastWrapperAttrs>) {
					...
					if (typeof options.content === "string") {
						contentNode = <p class="mui-toast__message">{options.content}</p>;
					} else {
						contentNode = m(options.content as m.Component<any>, {
					...
					return (
						<div
							...
							aria-atomic="true"
						>
							<div class="mui-toast__accent"></div>
							<div class="mui-toast__icon" aria-hidden="true">{meta.icon}</div>
							<div class="mui-toast__main">
								<div class="mui-toast__eyebrow">{meta.label}</div>
								<div class="mui-toast__header">
									{options.title ? <strong class="mui-toast__title">{options.title}</strong> : <span class="mui-toast__title mui-toast__title--empty">Notification</span>}
									<span class="mui-toast__pill">{appearance}</span>
								</div>
								<div class="mui-toast__body">{contentNode}</div>
								{vnode.attrs.duration > 0 ? (
									<div class="mui-toast__progress" aria-hidden="true">
										<span class="mui-toast__progress-bar" style={{ animationDuration: `${vnode.attrs.duration}ms` }}></span>
									</div>
								) : null}
							</div>
							<button
								type="button"
								class="mui-toast__close"
								aria-label="Close"
			...
			let bindOutsideClickTimer: number | null = null;
			...
			class ToastWrapperComponent implements m.ClassComponent<any> {
				view() {
